...
from localgrasp.LoG import lg_parse, LgNet, GraspGroup
...
def convert_traj_to_action(env, arm_jointindices, traj_qpos, gripper_pos) -> np.ndarray:
    if env.control_mode == "pd_joint_pos":
        return np.hstack([traj_qpos, gripper_pos])
    elif env.control_mode == "pd_joint_delta_pos":
        cur_joint_pos = np.array(env.agent.robot.get_qpos()[0, arm_jointindices])
        delta_action = traj_qpos - cur_joint_pos
        return np.concatenate(
            (
                inv_clip_and_scale_action(
                    delta_action,
                    env.agent.controller.configs["arm"].lower,
                    env.agent.controller.configs["arm"].upper
                ),
...
uenv.agent.robot.set_qpos(np.array([0, 0, 0, 0.2, 0, 0, 0, -1.032, 0, 0.955, -0.1, 1.57, 0, 0.015, 0.015]))
...
print("=== SETUP PLANNER ===")
print("arm_base_pose: ", uenv.agent.torso_lift_link.pose)
trans_world2arm = uenv.agent.torso_lift_link.pose.inv().to_transformation_matrix()
planner = mplib.Planner(
    urdf=format_path(uenv.agent.urdf_arm_ik_path),
    srdf=format_path(uenv.agent.urdf_arm_ik_path).replace(".urdf", ".srdf"),
    move_group="gripper_link",
)
OPEN_POS, CLOSE_POS = 1, -1
# extract mesh point cloud
# scene_full_pcd = uenv.gen_scene_pcd()
# scene_full_pcd_robot = transform_points(trans_world2arm, scene_full_pcd)
...
while init_plan_status != "Success" and plan_cnt + 2 <= plan_num:
    plan_cnt += 1
    ## grasp object
    pre_grasp_matrix = trans_world2arm @ gg_world_homo_close_prev_grasp[0][plan_cnt]
    ...
    plan_0 = planner.plan_screw(
        ...
        qpos=uenv.agent.robot.get_qpos().cpu().numpy()[0, [5]+list(range(7,15))],
        time_step=0.02,
...
if init_plan_status == "Success":
    ...
    for i in range(len(traj_0)):
        action = convert_traj_to_action(uenv, [5]+list(range(7,13)), traj_0[i], OPEN_POS)
        zeropad_action = np.zeros_like(env.action_space.sample())
        zeropad_action[0, :8] = action
        env.step(zeropad_action)
        env.render()

print(f"=== MOVE TO TARGET-GRASP ===")
grasp_matrix = trans_world2arm @ gg_world_homo_close_prev_grasp[1][plan_cnt]
...
if plan_1["status"] == "Success":
    ...
    for i in range(len(traj_1)):
        action = convert_traj_to_action(uenv, [5]+list(range(7,13)), traj_1[i], OPEN_POS)
        zeropad_action = np.zeros_like(env.action_space.sample())
        zeropad_action[0, :8] = action
        env.step(zeropad_action)
        env.render()
    ...
    for i in range(30):
        action = convert_traj_to_action(uenv, [5]+list(range(7,13)), traj_1[-1], CLOSE_POS)
        zeropad_action = np.zeros_like(env.action_space.sample())
        zeropad_action[0, :8] = action
        env.step(zeropad_action)
        env.render()
...
if plan_2["status"] == "Success":
    ...
    for i in range(len(traj_2)):
        action = convert_traj_to_action(uenv, [5]+list(range(7,13)), traj_2[i], CLOSE_POS)
        zeropad_action = np.zeros_like(env.action_space.sample())
        zeropad_action[0, :8] = action
        env.step(zeropad_action)
        env.render()
